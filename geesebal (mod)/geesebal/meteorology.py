...
def get_meteorology(image,time_start):
    ...
    PREVIOUS_IMAGE = DATASET.filterDate(PREVIOUS_TIME,TIME_START_NUM).limit(1,'system:time_start',False).first()

    NEXT_IMAGE = DATASET.filterDate(TIME_START_NUM,NEXT_TIME).limit(1,'system:time_start',False).first()
    ...
    i_sun_hour = i_lat_rad.expression('acos(- tan(lat)* tan(solar_dec))', {'lat' : i_lat_rad,'solar_dec' : solar_dec}).rename('sun_hour')
    ...
    # POTENTIAL ET [W M-2]
    pet = (NEXT_IMAGE.select('potential_evaporation_hourly')
           .subtract(PREVIOUS_IMAGE.select('potential_evaporation_hourly'))
           .multiply(DELTA_TIME).add(PREVIOUS_IMAGE.select('potential_evaporation_hourly'))
           .multiply((-1)*24*28.94).rename('Pet')) # mm/hr to w/m2 and upward fluxes have negative value 
    ...
    #CONCATENATES IMAGES
    col_meteorology = ee.Image.cat(rn24h, pet, tair_c, rh, wind_med, swdown24h)
