...
def fexp_spec_ind(image):
    ...
    #FOR FUTHER USE
    ndvi_pos = ndvi.updateMask(ndvi.gt(0)).rename('NDVI_pos')
    ndvi_neg = ndvi_pos.multiply(-1).rename('NDVI_neg')
    int_ = ee.Image(1).rename('int')
    sd_ndvi = ee.Image(1).rename('sd_ndvi')

    #ADD BANDS
    image = image.addBands([ndvi, evi, savi, lst, lai, e_0, e_NB, coords, ndvi_neg, ndvi_pos, int_, sd_ndvi, ndwi, brt_r])
    return image
...
def LST_DEM_correction(image, z_alt, T_air, UR, SUN_ELEVATION, hour, minutes):
    ...
    #SHUTTLEWORTH (2012)
    pres = image.expression('101.3 * ((293 - (0.0065 * Z))/ 293) ** 5.26 ', {'Z' : z_alt}).rename('P_ATM')

    #SATURATION VAPOR PRESSURE (es) [KPA]
    es = image.expression(' 0.6108 *(exp( (17.27 * T_air) / (T_air + 237.3)))', {'T_air': T_air}).rename('ES')
    ...
    W = image.expression('(0.14 * EA * PATM) + 2.1', {'PATM' : pres,'EA' : ea}).rename('W_ATM')
    ...
    tao_sw = image.expression(
       ...
       {'P' : pres,'W': W,'Kt' : ee.Number(1),'cos_theta' : cos_theta}).rename('Tao_sw')

    #AIR DENSITY [KG M-3]
    air_dens = image.expression('(1000* Pair)/(1.01*LST*287)',{'Pair': pres,'LST': image.select('T_LST')})    
    ...
    cos_zn =image.expression('-a +b*w_cos +c*w_sin',{'a': a,'b': b,'c': c,'w_cos': w.cos(),'w_sin': w.sin()})
    ...
    TS_DEM=image.expression(
        ...
        {'Temp_corr':Temp_corr,'Gsc':gsc,'dr':dr,'Transm_corr':tao_sw,
        'cos_zenith_flat':cos_theta,'cos_zn':cos_zn,'air_dens':air_dens}).rename('T_LST_DEM')
...
#INSTANTANEOUS OUTGOING LONG-WAVE RADIATION (Rl_up) [W/M2]
def fexp_radlong_up(image):
    #BROAD-BAND SURFACE THERMAL EMISSIVITY
    #TASUMI ET AL. (2003)
    #ALLEN ET AL. (2007)

   emi = image.expression('0.95 + (0.01 * LAI)', {'LAI' : image.select('LAI')})
   ...
   Rl_up = image.expression('emi * stefBol * (LST ** 4)', {'emi' : emi, 'stefBol': stefBol, 'LST': image.select('T_LST')}).rename('Rl_up')
...
#INSTANTANEOUS INCOMING SHORT-WAVE RADIATION (Rs_down) [W/M2]
def fexp_radshort_down(image, z_alt, T_air, UR, SUN_ELEVATION):

    #SOLAR CONSTANT
    gsc = ee.Number(1367) #[W/M2]
    ...
    Rs_down = image.expression(
       ...
       {'gsc' : gsc,'tao_sw': tao_sw,'dr' : dr,'cos_theta' : cos_theta}).rename('Rs_down')
...
#INSTANTANEOUS INCOMING LONGWAVE RADIATION (Rl_down) [W/M2]
#ALLEN ET AL (2007)
def fexp_radlong_down(image, n_Ts_cold):
    ...
    Rl_down = image.expression(
       ...
       {'log_taosw' : log_taosw,'stefBol': ee.Number(5.67e-8),'n_Ts_cold' : n_Ts_cold}).rename('Rl_down')
...
def fexp_radbalance(image):
    ...
    Rn = image.expression(
       ...
       {'alfa' : image.select('ALFA'),'Rs_down': image.select('Rs_down'),'Rl_down' : image.select('Rl_down'),
        'Rl_up' : image.select('Rl_up'),'e_0' : image.select('e_0')}).rename('Rn')
...
def fexp_soil_heat(image):
    ...
    G = image.expression(
        ...
        {'Rn' : image.select('Rn'),'NDVI': image.select('NDVI'),
         'ALFA': image.select('ALFA'),'T_LST': image.select('T_LST_DEM')}).rename('G')
...
def fexp_sensible_heat_flux(image, ux, UR, Rn24hobs, Pet, d_cold_pixel, d_hot_pixel, date_string, refpoly):
    
    #VEGETATION HEIGHTS  [M]
    n_veg_height = ee.Number(3) #CHECK!!!
    
    #WIND SPEED AT HEIGHT Zx [M]
    n_zx = ee.Number(2) #CHECK!!!
    
    #BLENDING HEIGHT [M]
    n_height = ee.Number(200) #CHECK!!!
    ...
    p_cold_pix_reduce = image.reduceRegion(
    ...
    #Rn
    n_Rn_cold = ee.Number(p_cold_pix_reduce.get('Rn'))
    #G
    n_G_cold = ee.Number(p_cold_pix_reduce.get('G'))  
    #Pet
    n_Pet_cold = ee.Number(Pet.reduceRegion(
        reducer = ee.Reducer.first(),
        geometry = p_cold_pix,
        scale = 11132,
        maxPixels = 9000000000).get('Pet'))
    ...
    p_hot_pix_reduce = image.reduceRegion(
        reducer = ee.Reducer.first(),
        geometry = p_hot_pix,
        scale = 30,
        maxPixels = 9000000000)
    #Rn
    n_Rn_hot = ee.Number(p_hot_pix_reduce.get('Rn'))
    #G
    n_G_hot = ee.Number(p_hot_pix_reduce.get('G'))
    #TS 
    n_Ts_hot = ee.Number(d_hot_pixel.get('temp'))
    #Pet
    n_Pet_hot = ee.Number(Pet.reduceRegion(
        reducer = ee.Reducer.first(),
        geometry = p_hot_pix,
        scale = 11132,
        maxPixels = 9000000000).get('Pet'))
    ...
    #AERODYNAMIC RESISTANCE TO HEAT TRANSPORT (rah) [S/M]

    #Z1 AND Z2 ARE HEIGHTS [M] ABOVE THE ZERO PLANE DISPLACEMENT
    #OF THE VEGETATION
    z1= ee.Number(0.1) #CHECK
    z2= ee.Number(2) #CHECK
    ...
    #SENSIBLE HEAT FLUX AT THE COLD PIXEL (H_cold)
    n_H_cold = (ee.Number(n_Rn_cold).subtract(ee.Number(n_G_cold)).subtract(ee.Number(n_Pet_cold))
                .clamp(ee.Number(0),ee.Number(ee.Number(n_Rn_cold).subtract(ee.Number(n_G_cold)))))
    ...
    #ITERATIVE VARIABLES
    n = ee.Number(1)
    n_dif_cold = ee.Number(1)
    n_dif_hot = ee.Number(1)
    n_dif_min = ee.Number(0.1)
    list_dif_cold = ee.List([])
    list_dif_hot = ee.List([])
    list_dT_cold = ee.List([])
    list_dT_hot = ee.List([])
    list_rah_cold = ee.List([])
    ...
    for n in range(15):
        ...
        n_dT_cold = (n_H_cold.multiply(n_rah_cold)).divide(n_ro_cold.multiply(n_Cp))  
        ...
        i_psim_200 = img.expression(
                '-5*(height/i_L_int)', {'height' : ee.Number(200),'i_L_int': i_L_int}).rename('psim_200')
        i_psih_2 = img.expression(
                '-5*(height/i_L_int)',{'height' : ee.Number(2),'i_L_int': i_L_int}).rename('psih_2')
        i_psih_01 = img.expression(
                '-5*(height/i_L_int)',{'height' : ee.Number(0.1),'i_L_int': i_L_int}).rename('psih_01')

        #FOR DIFFERENT HEIGHT
        i_x200 = i_L_int.expression(
                '(1-(16*(height/i_L_int)))**0.25',
                {'height' : ee.Number(200),'i_L_int': i_L_int}).rename('i_x200')
        i_x2 = i_L_int.expression(
                '(1-(16*(height/i_L_int)))**0.25',
                {'height' : ee.Number(2),'i_L_int': i_L_int}).rename('i_x2')
        i_x01 = i_L_int.expression(
                '(1-(16*(height/i_L_int)))**0.25',
                {'height' : ee.Number(0.1),'i_L_int': i_L_int})
        ...
        if n == 1:
            n_dT_cold_old = n_dT_cold
            n_dT_hot_old = n_dT_hot
            
            n_rah_cold_old = n_rah_cold
        ...
        if n > 1:
            n_dT_cold_abs = n_dT_cold.abs()
            n_dT_hot_abs = n_dT_hot.abs()
            
            n_dT_cold_old_abs = n_dT_cold_old.abs()
            n_dT_hot_old_abs = n_dT_hot_old.abs()
            
            n_rah_cold_abs = n_rah_cold.abs()
            n_rah_hot_abs = n_rah_hot.abs()
            
            n_rah_cold_old_abs = n_rah_cold_old.abs()
            ...
            n_dif_cold = (n_dT_cold_abs.subtract(n_dT_cold_old_abs).add(n_rah_cold_abs).subtract(n_rah_cold_old_abs)).abs()
            n_dif_hot = (n_dT_hot_abs.subtract(n_dT_hot_old_abs).add(n_rah_hot_abs).subtract(n_rah_hot_old_abs)).abs()
            
        #INSERT EACH ITERATION VALUE INTO A LIST
        list_coef_a = list_coef_a.add(n_coef_a)
        list_coef_b = list_coef_b.add(n_coef_b)
        
        list_dT_cold = list_dT_cold.add(n_dT_cold)
        list_dT_hot = list_dT_hot.add(n_dT_hot)
        
        list_rah_cold = list_rah_cold.add(n_rah_cold)
        list_rah_hot = list_rah_hot.add(n_rah_hot)
        
        list_dif_cold = list_dif_cold.add(n_dif_cold)
        list_dif_hot = list_dif_hot.add(n_dif_hot)
        
    #=========END ITERATION =========#
    i_rah_final = i_rah.rename('rah') #[S/M]
