...
def fexp_hot_pixel(image, refpoly, p_lowest_NDVI, p_hottest_Ts):
    ...
    #IDENTIFY THE LOW % NDVI PIXELS
    d_perc_down_ndvi = image.select('NDVI_pos').reduceRegion(
    ...
    #GET VALUE
    n_perc_low_NDVI = ee.Number(d_perc_down_ndvi.get('NDVI_pos'))
    
    #UPDATE MASK WITH NDVI VALUES
    i_low_NDVI = image.updateMask(image.select('NDVI_pos').lte(n_perc_low_NDVI))
